...
@class OHomeController;
@interface FINaviController ()<UINavigationBarDelegate,UIGestureRecognizerDelegate>

@end

@implementation FINaviController

+ (void)load
{
    if (self == [FINaviController class]) {
        UINavigationBar *navBar = [UINavigationBar appearance];
        UIBarButtonItem *barItem = [UIBarButtonItem appearance];
        UIColor *whcolor = [UIColor whiteColor];
//        [navBar setBackgroundImage:MLBImgName(@"nvbg") forBarMetrics:UIBarMetricsDefault];
//        [navBar setShadowImage:nil];
        ...
        navBar.barTintColor = [UIColor mainBlackColor];
        navBar.tintColor = [UIColor whiteColor];
        
        [navBar setTitleTextAttributes:@{NSForegroundColorAttributeName:whcolor}];
        
        [navBar setTitleTextAttributes:@{
                                         NSForegroundColorAttributeName :whcolor ,
                                         NSFontAttributeName : FIFont(18.f)
                                         }];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.childViewControllers.count != 0) {
        ...
        [leftBtn setImage:[UIImage imageNamed:@"MBack"] forState:UIControlStateNormal];
//        [leftBtn sizeToFit];
        /** 想让 导航栏的左按钮向左偏一点的方法 */
